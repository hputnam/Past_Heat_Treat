...
```{r, warning=FALSE, message=FALSE}
path.p<-"data/pr_sdr/runP/test" #location of files
```
...
colnames(Photo.R) <- c("Date", "Plate","Chamber.ID","Light", "Intercept", "umol.L.min")

Photo.Rb <- data.frame(matrix(NA, ncol=6))
colnames(Photo.Rb) <- c("Date", "Plate","Chamber.ID","Light", "Intercept", "umol.L.min")
...
for(file in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    Photo.Rb[j,5] <- as.character(1) #stores the chamber ID
    Photo.Rb[j,6:7] <- model$allRegs[1,c(4,5)] #inserts slope and intercept in the dataframe
