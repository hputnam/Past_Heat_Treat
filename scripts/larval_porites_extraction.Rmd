...
path.p<-"data/pr_sdr/runP/test" #location of file
```
...
for (i in Light_Values) { #in every file, for each light value 
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    pdf(paste0("data/pr_sdr/P_Plots/",date[i],"_Run",run[file], "_", rename[j],"_light", Light_Values[i],"_regression.pdf")) #generate output file names
    plot(model)
    dev.off()
