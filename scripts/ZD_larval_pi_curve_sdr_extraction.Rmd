...
rename_trunc <- rename[9:12] # the rename cannot be truncated because the data are running in all columns of the data sheet and must have the same order as the data sheet. It is not filtering by well using the rename vector 
...
Convert air saturation rates to concentration values using respR package. 
```{r}
oxygen<-oxygen%>%
 mutate(P.umol.L.min=respR::convert_DO(P.umol.L.min, from = "%Air", to = "umol/L", t = 28, S = Salinity))
```
